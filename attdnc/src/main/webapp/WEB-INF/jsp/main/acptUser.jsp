...
<head>
<meta charset="UTF-8">
<title>근태관리 > 가입승인</title>
<link href="../css/acptUser.css" rel="stylesheet" type="text/css">
...
<body>
	...
	$(document).ready(function(){
		...
		function init() {
			...
			request("./getAcptYetUserList.do", params, function callback(response, status) {
				if (response.RES_CD == "000") {
					console.log("response: " + response);
					console.log("userList: " + response.userList[0].USER_ID);
					drawUserList(response.userList)
				} else if (response.RES_CD == "001") {
		...
		function drawUserList(userList) {
			...
			for (i = 0; i < userList.length; i++) {
				...
				html += '	<td>';
				html += '		<input type="button" value="승인" onclick="btnAcpt('+ userList[i].USER_NO +', 1);">';
				html += '	</td>';
							
				html += '	<td>';
				html += '		<input type="button" value="반려" onclick="btnAcpt('+ userList[i].USER_NO +', 2);">';
				html += '	</td>';
		...
		// 승인/반려 버튼 클릭
		function btnAcpt(userNo, type) { // type : 1 = 승인, 2 = 반려
			...
			request("./updateUserAcptType.do", params, function callback(response, status) {
				if (response.RES_CD == "000") {
